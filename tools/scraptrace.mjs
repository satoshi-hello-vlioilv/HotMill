...
const out = await page.evaluate((EPS) => {
  ...
  /* この評価器は «先端と後端の両方を、それぞれ複数カットする» 経路を見るためのもの。
   * カット数は端部欠陥の長さ ＝ 伸び（h0/h）で決まるので、«目標板厚まで届くロット» を
   * 選ぶ必要がある。板長を最大にすると、板が伸びて出側テーブルに載らなくなり
   * 目標より手前で止まる（伸びが足りずカットが 2 回で終わる）ので、最小の板長を据える。
   * 評価器が «たまたまの既定» にも «届かないロット» にも依存しないようにする。 */
  const setLot = () => {
