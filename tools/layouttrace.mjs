...
const out = await page.evaluate(() => {
  ...
  R.table = {};
  {
    ...
    if (SEC) {
      ...
      const Lay0 = window.__LAYOUT, bad = [], stray = [];
      for (const s of SEC) {
        ...
        for (const g of miss) stray.push(`${s.name}@${g.toFixed(0)}`);
      ...
      /* ローラは 1 本も欠けていないこと。設備（X 線板厚計など）を足すために抜くのは
       * «図面のピッチを崩す» ことなので、抜かずに納まっているかをここで縛る。 */
      ok('図面のローラが 1 本も欠けていない', stray.length === 0, stray.join(' ') || '欠け 0');
      ...
      ok('図面のローラ総数と一致（E-1 / E-2 は 1 段 OS/DS の 2 本）', placed === total,
         `${placed} / 図面 ${total} 本`);
      /* ピッチも図面どおりであること（区分ごとの実配置を図面の指定から作った列と突き合わせる）。 */
      const pbad = [];
      for (const s of SEC) {
        const gen = Lay0.section(s).xs;
        for (let i = 1; i < gen.length; i++) {
          const p0 = Math.abs(gen[i] - gen[i - 1]);
          const got = xs.filter(x => gen.some(g => Math.abs(g - x) < 1)).sort((a, b) => a - b);
          if (got.length !== gen.length) { pbad.push(s.name); break; }
          if (Math.abs(Math.abs(got[i] - got[i - 1]) - p0) > 1) { pbad.push(`${s.name}@${i}`); break; }
        }
      }
      ok('図面のローラピッチが区分ごとに一致', pbad.length === 0, pbad.join(' ') || `${SEC.length} 区分すべて一致`);
